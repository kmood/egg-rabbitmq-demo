...
class HomeController extends Controller {
  async index() {
    ...
    for (let i = 0; i < 5; i++) {
      // 常规
      await app.rabbit.publish(app.config.rabbit.exchanges.message.name, 'wechat.chatroom.MOCK_CHATROOM', Buffer.from('微信普通群A' + i));
      await app.rabbit.publish(app.config.rabbit.exchanges.message.name, 'wechat.person.MOCK_CUSROMER', Buffer.from('微信个人B' + i));
      await app.rabbit.publish(app.config.rabbit.exchanges.message.name, 'wechat.im_chatroom.MOCK_IM', Buffer.from('微信企业群C' + i));
      await app.rabbit.publish(app.config.rabbit.exchanges.message.name, 'wxwork.person.MOCK_CUSROMER', Buffer.from('企业个人D' + i));
      await app.rabbit.publish(app.config.rabbit.exchanges.message.name, 'line.person.MOCK_CUSROMER', Buffer.from('line个人E' + i));
